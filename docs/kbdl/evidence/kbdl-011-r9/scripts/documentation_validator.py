...
if len(dids)!=15 or len(set(dids))!=15 or dids!=[f'KBDL-DEC-{i:03}' for i in range(1,16)]:err['decision IDs'].append(f'count/order/unique defect: {dids}')
for r in rows:
...
if re.search(r'first seven steps of a ten-step|current (?:approved )?roadmap.{0,30}ten-step|validate KBDL-011-R8 next',joined,re.I):err['stale roadmap wording'].append('stale roadmap phrase')
if re.search(r'(?i)(?:KBDL project is complete|completion approved|implementation conformance\s*(?:is|:)\s*(?:verified|pass))',joined):err['premature readiness/completion claims'].append('premature completion/conformance claim')
