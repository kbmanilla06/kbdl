#!/usr/bin/env python3
"""Run real R15 source mutations against the production validator in isolated copies."""
from pathlib import Path
import csv,json,re,shutil,subprocess,sys,tempfile
ROOT=Path(__file__).resolve().parents[5];PK=ROOT/'docs/kbdl/evidence/kbdl-011-r15';OUT=PK/'negative-tests';OUT.mkdir(parents=True,exist_ok=True)
CASES={
...
def mutate(root,case):
 d=root/'docs/kbdl';meta=d/'traceability-metadata.csv';rules=d/'evidence/kbdl-011-r15/source-rules.csv';rec=d/'evidence/kbdl-011-authority-recovery/authority-recovery-ledger.csv'
 if case=='ledger-normative-lifecycle':rewrite_csv(meta,lambda rs:row(rs,'KBDL-VAL-001').update({'Lifecycle status':'Recommended'}))
  ...
  rewrite_csv(meta,f)
def main():
 baseline_dir=Path(tempfile.mkdtemp(prefix='kbdl-r15-base-'));baseline=subprocess.run([sys.executable,str(PK/'scripts/production_validator.py'),'--root',str(ROOT),'--output',str(baseline_dir)],text=True,capture_output=True);shutil.rmtree(baseline_dir)
 ...
 for case,want in CASES.items():
  tmp=Path(tempfile.mkdtemp(prefix='kbdl-r15-mut-'));fixtures.append(tmp);fixture=tmp/'repo';shutil.copytree(ROOT/'docs',fixture/'docs');shutil.copytree(ROOT/'.git',fixture/'.git');mutate(fixture,case);out=tmp/'out'
  p=subprocess.run([sys.executable,str(fixture/'docs/kbdl/evidence/kbdl-011-r15/scripts/production_validator.py'),'--root',str(fixture),'--output',str(out)],text=True,capture_output=True)
  cats=set(re.findall(r'^([A-Z0-9_]+):',p.stderr,re.M));ok=p.returncode!=0 and want in cats;wrong=not ok
  (OUT/f'{case}.txt').write_text(f'Case: {case}\nMutation: actual source file changed in isolated repository copy\nProduction validator: docs/kbdl/evidence/kbdl-011-r15/scripts/production_validator.py\nExpected category: {want}\nExit code: {p.returncode}\nDetected categories: {"; ".join(sorted(cats)) or "None"}\nIntended detection: {ok}\nFixture destroyed: yes\n\nSTDOUT\n{p.stdout}\nSTDERR\n{p.stderr}')
