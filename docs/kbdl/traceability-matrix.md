...
unrecovered. R12 is the current semantic remediation pending planning-agent
validation.
...
- **Lifecycle status:** All Approved.
- **Provenance:** All Confirmed.
- **Validation status (per-ID):** `001`, `002`: Verified; `003`, `004`: Not verified after R12 source-independence correction.
...
- **Lifecycle status:** All Approved.
- **Provenance:** All Confirmed.
- **Validation status (per-ID):** `005`, `007`: Verified; `006`, `008`: Not verified.
- **Authority:** Explicit mandatory KBDL-011 prompt clauses approved by the project owner.
...
without reconstructing historical prompt approvals. R12 removes the R11
false-positive restorations for `VAL-003`, `VAL-004`, and `VAL-006`; `VAL-007`
remains Verified for its complete documentation method. `VAL-008`, `010`, and
`011` remain Not verified. The documentation-only candidate recommendation is
`NOT READY`, pending planning-agent validation and without readiness or
completion approval.
