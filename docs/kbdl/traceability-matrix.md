...
- **Known limitation:** None identified.
- **Related decision:** Not applicable
- **Notes:** The category-label wording error ("×5 sub-roles ×5 families") in the original parity matrix — which contradicted its own correct 4×6=24 total — is corrected under KBDL-004-R1; the total of 72 was independently re-verified, not merely preserved. Under KBDL-004-R2, the alias *definition* was reconciled to one model across all documents: an alias is a role with no separate definition of its own (Keyboard focus, Neutral status); a role whose *value* merely reuses another role's color (Text-link states, Selection text, Muted metadata) is not an alias. This did not change the totals (still 72/72/72/2), only removed an inconsistent claim in `semantic-roles.md` that had counted Text-link states as an alias while simultaneously counting it among the 10 distinct Actions roles.
...
- **Validation method:** WCAG contrast calculation (completed for status pairs and the opaque gradient caption substitute); manual review (color-expression — not yet performed by the project owner).
